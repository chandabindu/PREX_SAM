...
lumiPrimaryGeneratorMessenger::lumiPrimaryGeneratorMessenger(lumiPrimaryGeneratorAction* pga)
  ...
  setRasterWidth = new G4UIcmdWithADouble("/lumi/gun/rasterwidth",this);
  setRasterWidth->SetParameterName("raster_width",true);
  setRasterWidth->SetDefaultValue(0);
  setRasterWidth->AvailableForStates(G4State_PreInit,G4State_Idle);

  setRasterHeight = new G4UIcmdWithADouble("/lumi/gun/rasterheight",this);
  setRasterHeight->SetParameterName("raster_height",true);
  setRasterHeight->SetDefaultValue(0);
  setRasterHeight->AvailableForStates(G4State_PreInit,G4State_Idle);
...
lumiPrimaryGeneratorMessenger::~lumiPrimaryGeneratorMessenger(){
  ...
  delete setRasterWidth;
  delete setRasterHeight;
...
void lumiPrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String newValue){
  ...
  if(command==setRasterWidth){
    primaryGenAction->SetRasterWidth(setRasterWidth->GetNewDoubleValue(newValue));
  }

  if(command==setRasterHeight){
    primaryGenAction->SetRasterHeight(setRasterHeight->GetNewDoubleValue(newValue));
  }
